...
export function OwnerQuickActionsRow({ lang, showRecordExpense }: Props) {
  ...
  return (
    <OwnerDashboardSection title={t(lang, "ownerSectionQuickActions")} collapsible={false}>
      <div className="-mx-1 flex gap-2 overflow-x-auto pb-1">
        {actions.map(({ to, label, Icon }) => (
          <Link
            key={to}
            to={to}
            className="inline-flex min-h-[48px] shrink-0 items-center gap-2 rounded-2xl border-2 border-slate-200 bg-white px-4 py-2.5 text-sm font-black text-slate-800 shadow-sm transition-colors hover:border-waka-300 hover:bg-waka-50 active:scale-[0.98]"
          >
