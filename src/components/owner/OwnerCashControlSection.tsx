  ...
  return (
    <section className="rounded-2xl border border-border/90 bg-card p-4 shadow-sm">
      <div className="flex items-start justify-between gap-2">
        <div>
          <h2 className="text-base font-black text-foreground">{t(lang, "ownerCashTitle")}</h2>
          <p className="text-xs font-semibold text-muted-foreground">{t(lang, "ownerCashSub")}</p>
        </div>
      ...
      <dl className="mt-3 grid gap-2 sm:grid-cols-3">
        ...
        <div className="rounded-xl bg-muted px-2.5 py-2">
          <dt className="text-[10px] font-bold uppercase text-muted-foreground">{t(lang, "ownerCashDayVariance")}</dt>
          <dd
            className={`mt-0.5 text-sm font-black tabular-nums ${
              cash.latestDayVarianceUgx != null && cash.latestDayVarianceUgx < 0 ? "text-rose-700" : "text-foreground"
            }`}
          >
      ...
      <dl className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-4">
        <div className="rounded-lg bg-muted px-2 py-1.5 text-[11px]">
          <dt className="font-semibold text-muted-foreground">{t(lang, "ownerCashOwnerInjection")}</dt>
          <dd className="font-black tabular-nums">UGX {cash.ownerInjectionsUgx.toLocaleString()}</dd>
        </div>
        <div className="rounded-lg bg-muted px-2 py-1.5 text-[11px]">
          <dt className="font-semibold text-muted-foreground">{t(lang, "ownerCashOwnerWithdrawal")}</dt>
          <dd className="font-black tabular-nums text-rose-700">UGX {cash.ownerWithdrawalsUgx.toLocaleString()}</dd>
        </div>
        <div className="rounded-lg bg-muted px-2 py-1.5 text-[11px]">
          <dt className="font-semibold text-muted-foreground">{t(lang, "ownerCashBankDeposit")}</dt>
          <dd className="font-black tabular-nums">UGX {cash.bankDepositsUgx.toLocaleString()}</dd>
        </div>
        <div className="rounded-lg bg-muted px-2 py-1.5 text-[11px]">
          <dt className="font-semibold text-muted-foreground">{t(lang, "ownerCashSafeTransfer")}</dt>
      ...
      {cash.topCashierShortages.length > 0 ? (
        <div className="mt-3">
          <p className="text-[10px] font-black uppercase tracking-wide text-muted-foreground">{t(lang, "ownerCashTopShortages")}</p>
      ...
      {cash.shiftVariances.length > 0 ? (
        <div className="mt-4">
          <p className="text-xs font-black uppercase tracking-wide text-muted-foreground">{t(lang, "ownerCashShiftVariances")}</p>
          <ul className="mt-2 space-y-1">
            {cash.shiftVariances.map((sv) => (
              <li
                key={sv.shiftId}
                className="flex justify-between rounded-lg border border-border px-3 py-2 text-xs"
              >
                <span className="font-semibold text-foreground">{sv.label}</span>
                <span
      ...
      {cash.adjustmentFeed.length > 0 ? (
        <div className="mt-4">
          <p className="text-xs font-black uppercase tracking-wide text-muted-foreground">{t(lang, "ownerCashAdjustmentFeed")}</p>
          <ul className="mt-2 space-y-1">
            {cash.adjustmentFeed.map((row) => (
              <li key={row.id} className="rounded-lg border border-border px-3 py-2 text-xs">
      ...
      {cash.floatVerificationFeed.length > 0 ? (
        <div className="mt-4">
          <p className="text-xs font-black uppercase tracking-wide text-muted-foreground">{t(lang, "ownerCashFloatFeed")}</p>
          <ul className="mt-2 space-y-1">
