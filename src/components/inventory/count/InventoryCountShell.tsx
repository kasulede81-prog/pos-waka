  ...
  const body = (
    <>
      {variant === "modal" || title ? (
        <header
          className={clsx(
            variant === "modal" && "shrink-0 border-b border-border/60 bg-card/95 px-4 pb-4 pt-4 backdrop-blur-md sm:px-5",
          )}
        >
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1 pt-0.5">
              ...
              <h2
                id={titleId}
                className={clsx("font-black tracking-tight text-foreground", variant === "modal" ? "truncate text-lg" : "text-xl")}
              >
                {title}
              </h2>
            </div>
      ...
      <form className={clsx("flex flex-col", variant === "modal" && "min-h-0 flex-1")} onSubmit={onSubmit} noValidate>
        <div
          className={clsx(
            "space-y-5",
