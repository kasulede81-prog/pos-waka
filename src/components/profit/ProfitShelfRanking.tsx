  ...
  return (
    <section className="rounded-2xl border border-border/90 bg-card shadow-sm">
      <div className="border-b border-border px-3 py-2.5">
        <h3 className="text-xs font-black text-foreground">{t(lang, "profitShelfPerformance")}</h3>
      </div>
          ...
          return (
            <li key={g.categoryKey}>
              <button
                ...
                className="flex w-full min-h-[52px] items-center gap-2.5 px-3 py-2.5 text-left active:bg-muted"
              >
                <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-muted text-base">
                  {medal ?? icon}
                </span>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-black text-foreground">{g.categoryLabel}</p>
                ...
                <div className="shrink-0 text-right">
                  ...
                  <p className="text-[10px] font-bold text-muted-foreground">{pct.toFixed(0)}%</p>
                </div>
