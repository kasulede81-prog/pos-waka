    ...
    <AppModalOverlay className="z-[54] flex items-end bg-foreground/40 backdrop-blur-[2px]" clearNav={false}>
      ...
      <div className="relative z-[55] max-h-[min(85dvh,40rem)] w-full overflow-y-auto rounded-t-[1.75rem] border border-border bg-card px-4 pb-[calc(var(--waka-bottom-nav-h)+var(--waka-safe-bottom)+1rem)] pt-3 shadow-2xl">
        ...
        <div className="flex items-center gap-3">
          <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-muted text-sm font-black text-muted-foreground">
            {productInitials(product.name)}
          </span>
          <div className="min-w-0">
            <p className="truncate text-base font-black text-foreground">{product.name}</p>
            <p className="text-xs font-semibold text-muted-foreground">{product.shelfLabel}</p>
          </div>
        </div>
        ...
        <dl className="mt-4 space-y-2 rounded-xl bg-muted p-3">
          {[
            ...
            <div key={row.label} className="flex items-center justify-between gap-2 text-sm">
              ...
              <dd className={`font-black tabular-nums ${row.valueClass ?? "text-foreground"}`}>{row.value}</dd>
            </div>
          ))}
        </dl>
        ...
        <div className="mt-3 grid grid-cols-2 gap-2">
          <Link
            ...
            onClick={onClose}
            className="flex min-h-[44px] items-center justify-center rounded-xl border border-border text-sm font-bold text-foreground active:bg-muted"
          >
