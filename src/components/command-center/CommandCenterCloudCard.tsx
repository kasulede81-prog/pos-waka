...
export function CommandCenterCloudCard({ lang, cloud, devicesOnline, devicesTotal }: Props) {
  ...
  return (
    <section className="overflow-hidden rounded-3xl border border-stone-200/90 bg-gradient-to-br from-white to-stone-50/80 p-4 shadow-sm sm:p-5">
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h2 className="text-sm font-black text-stone-950 sm:text-base">{t(lang, "cmdCenterCloudTitle")}</h2>
      ...
      <p className="mt-3 flex items-start gap-1.5 text-[11px] font-semibold text-stone-600">
      ...
      <Link
        to="/settings/devices"
        className="mt-4 flex min-h-[44px] w-full items-center justify-center rounded-2xl border-2 border-stone-200 bg-white px-4 text-sm font-black text-stone-900"
      >
...
function Metric({
  ...
  return (
    <div className={`rounded-2xl px-2.5 py-2 ${warn ? "bg-amber-50" : highlight ? "bg-waka-50" : "bg-white ring-1 ring-stone-100"}`}>
      <p className="text-[10px] font-bold uppercase tracking-wide text-stone-500">{label}</p>
      <p className={`mt-0.5 font-black tabular-nums text-stone-950 ${small ? "text-[11px]" : "text-sm"}`}>{value}</p>
    </div>
  );
}
