  ...
  return (
    <section>
      <div className="mb-2 flex items-end justify-between gap-2">
        <div>
          <h2 className="text-sm font-black text-stone-950 sm:text-base">{t(lang, "cmdCenterOverviewTitle")}</h2>
      ...
      <div className="grid grid-cols-2 gap-2 sm:gap-2.5 lg:grid-cols-3">
        {cards.map((card) => (
          <div
            key={card.id}
            className="flex min-h-[96px] flex-col justify-between rounded-2xl border border-stone-200/90 bg-white p-2.5 shadow-sm"
          >
            <p className="text-[10px] font-bold uppercase tracking-wide text-stone-500">{t(lang, card.labelKey)}</p>
            <p className={clsx("truncate text-base font-black tabular-nums sm:text-lg", card.valueClass ?? "text-stone-950")}>
              {card.value}
            </p>
