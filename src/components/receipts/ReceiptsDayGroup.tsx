...
export function ReceiptsDayGroup({
  ...
  const shellClass =
    tone === "pending"
      ...
      : "group overflow-hidden rounded-[1.35rem] border border-stone-200/90 bg-white shadow-waka-sm open:ring-1 open:ring-waka-100";

  const chevronClass = tone === "pending" ? "text-amber-600" : "text-stone-400";
      ...
      {open ? (
        <div
          className={`space-y-2 border-t px-3 py-3 sm:px-4 ${
            tone === "pending" ? "border-amber-100" : "border-stone-100 bg-stone-50/50"
          }`}
        >
          {onDownloadDay ? (
            <div className="flex justify-end">
              <button
                ...
                className="inline-flex min-h-[44px] items-center gap-1.5 rounded-xl bg-white px-3 text-xs font-black text-waka-700 ring-1 ring-stone-200"
              >
