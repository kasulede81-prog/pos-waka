...
export function AdminCrashSummaryPanel({ previewMode = false }: Props) {
  ...
  return (
    <section className="rounded-2xl border border-stone-200 bg-white p-4 shadow-sm">
      <h2 className="text-sm font-black text-stone-900">Crash monitoring</h2>
      <p className="mt-0.5 text-xs text-stone-500">Read-only · complements Sentry · from app reports.</p>
      <p className="mt-2 font-mono text-2xl font-black text-stone-900">{summary.crashes_today}</p>
      <p className="text-xs font-semibold text-stone-500">Crashes today (Kampala)</p>

      {summary.by_version.length > 0 ? (
        <div className="mt-3">
          <p className="text-xs font-black text-stone-700">By version</p>
          <ul className="mt-1 space-y-1 text-xs">
            {summary.by_version.map((v) => (
              <li key={v.version} className="flex justify-between rounded-lg bg-stone-50 px-2 py-1">
                <span>{v.version}</span>
      ...
      {summary.by_shop.length > 0 ? (
        <div className="mt-3">
          <p className="text-xs font-black text-stone-700">Affected shops</p>
          <ul className="mt-1 space-y-1 text-xs">
