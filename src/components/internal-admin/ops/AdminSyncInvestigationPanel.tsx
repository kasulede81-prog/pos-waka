...
export function AdminSyncInvestigationPanel({ detail, diagnosticsPending }: Props) {
  ...
  return (
    <section className="rounded-2xl border border-border bg-card p-4 shadow-sm">
      <h2 className="text-sm font-black text-foreground">Sync investigation</h2>
      <p className="mt-0.5 text-xs text-muted-foreground">Cloud queue state — no database access required.</p>
      ...
      {detail.devices.length > 0 ? (
        <div className="mt-3">
          <p className="text-xs font-black text-muted-foreground">Devices</p>
          <ul className="mt-1 space-y-1 text-xs text-muted-foreground">
            {detail.devices.map((d) => (
              <li key={d.id} className="rounded-lg bg-muted px-2 py-1.5">
...
function Row({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className={`flex justify-between gap-2 rounded-xl px-3 py-2 ${highlight ? "bg-amber-50" : "bg-muted"}`}>
      <dt className="font-semibold text-muted-foreground">{label}</dt>
      <dd className="max-w-[55%] text-right font-mono text-xs font-black text-foreground">{value}</dd>
    </div>
  );
}
