...
export function RestaurantRunningOrderPanel({
  ...
  return (
    <aside
      className={clsx(
        "flex min-h-0 flex-col border-l border-border bg-muted",
        className,
      )}
    >
      <div className="shrink-0 border-b border-border bg-card px-3 py-2">
        <p className="text-xs font-black uppercase text-muted-foreground">{t(lang, "restaurantRunningOrder")}</p>
      ...
      <div className="grid shrink-0 grid-cols-[2rem_1fr_auto] gap-x-2 border-b border-border bg-muted px-2 py-1 text-[9px] font-black uppercase text-muted-foreground">
        <span>Q</span>
      ...
      <ul className="min-h-0 flex-1 overflow-y-auto">
        {lines.length === 0 ? (
          <li className="px-3 py-8 text-center text-xs font-semibold text-muted-foreground">{t(lang, "restaurantOrderEmpty")}</li>
        ) : (
          lines.map((line) => {
            ...
            return (
              <li key={line.id ?? line.productId}>
                <button
                  ...
                  className="grid w-full grid-cols-[2rem_1fr_auto] items-center gap-x-2 border-b border-border px-2 py-2 text-left hover:bg-card"
                >
                  <span className="text-sm font-black tabular-nums text-foreground">{qtyLabel}</span>
      ...
      <div className="shrink-0 space-y-0.5 border-t border-border bg-card px-3 py-2 text-xs">
