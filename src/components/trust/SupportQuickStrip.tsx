...
export function SupportQuickStrip({ lang }: Props) {
  return (
    <details className="group rounded-2xl border border-orange-200/90 bg-gradient-to-br from-orange-50 to-white shadow-sm">
      <summary className="flex cursor-pointer list-none items-center gap-3 p-4 [&::-webkit-details-marker]:hidden">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 text-sm font-medium text-stone-600">{t(lang, "supportQuickExpandHint")}</p>
        </div>
        <ChevronDown
          className="h-5 w-5 shrink-0 text-orange-800 transition-transform group-open:rotate-180"
          aria-hidden
        />
      </summary>
      <div className="border-t border-orange-100/90 px-4 pb-4 pt-3">
        <p className="text-sm font-medium text-stone-600">{t(lang, "supportQuickSub")}</p>
