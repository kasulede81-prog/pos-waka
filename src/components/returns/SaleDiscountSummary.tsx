    ...
    <div className={`rounded-xl border border-violet-100 bg-violet-50/60 px-3 py-2 text-xs ${className}`}>
      ...
      <dl className="mt-1 space-y-0.5">
        <div className="flex justify-between gap-2">
          <dt className="font-semibold text-slate-700">{t(lang, "checkoutSubtotalLabel")}</dt>
          <dd className="font-bold text-slate-900">UGX {breakdown.listSubtotalUgx.toLocaleString()}</dd>
        </div>
        {breakdown.lineDiscountsUgx > 0 ? (
          <div className="flex justify-between gap-2">
            <dt className="font-semibold text-slate-700">{t(lang, "checkoutLineDiscountsLabel")}</dt>
            <dd className="font-bold text-rose-800">− UGX {breakdown.lineDiscountsUgx.toLocaleString()}</dd>
          </div>
        ) : null}
        {breakdown.cartDiscountUgx > 0 ? (
          <div className="flex justify-between gap-2">
            <dt className="font-semibold text-slate-700">{t(lang, "checkoutCartDiscountLabel")}</dt>
        ...
        <div className="flex justify-between gap-2 border-t border-violet-100 pt-1">
          <dt className="font-bold text-slate-800">{t(lang, "receiptFinalPaidLabel")}</dt>
          <dd className="font-black text-slate-950">UGX {breakdown.finalTotalUgx.toLocaleString()}</dd>
        </div>
      </dl>
    </div>
  );
