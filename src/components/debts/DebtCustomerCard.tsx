    ...
    <article className="rounded-2xl border border-stone-200/90 bg-white p-3 shadow-sm transition-all active:scale-[0.99] motion-reduce:active:scale-100">
      <button type="button" onClick={onOpenDetail} className="flex w-full items-start gap-2.5 text-left">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className={clsx("mt-1 text-base font-black tabular-nums", hasBalance ? "text-waka-700" : "text-stone-500")}>
            UGX {customer.debtBalanceUgx.toLocaleString()}
          </p>
          <div className="mt-1 flex flex-wrap gap-x-3 text-[10px] font-medium text-stone-500">
            {meta.lastPayment ? (
              <span>
      ...
      <div className="mt-2.5 flex gap-1.5">
        {customer.phone ? (
          <a
            ...
            className="inline-flex min-h-[36px] flex-1 items-center justify-center gap-1 rounded-xl border border-stone-200 text-xs font-bold text-stone-800 active:bg-stone-50"
          >
        ...
        <button
          ...
          className="flex min-h-[36px] min-w-[36px] items-center justify-center rounded-xl border border-stone-200 text-stone-700 active:bg-stone-50"
