...
function hasLegacyTrialRow(snapshot: SubscriptionSnapshot): boolean {
  ...
  return trialLike && normalizePlanCode(row.plan_code) !== "free";
...
export function OfficePremiumSection({ lang }: { lang: Language }) {
  ...
  const legacyTrial = authMode === "supabase" && hasLegacyTrialRow(snapshot);
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="min-w-0">
          ...
          {legacyTrial ? (
            ...
            <p className="mt-2 text-xs font-semibold text-orange-100/95">{t(lang, "officePremiumLegacyTrialHint")}</p>
