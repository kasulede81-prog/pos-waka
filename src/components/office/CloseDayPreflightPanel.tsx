...
export function CloseDayPreflightPanel({ lang, snapshot, loading }: Props) {
  ...
  return (
    <section className="rounded-3xl border border-border bg-card p-4 shadow-waka-sm">
      ...
      {snapshot && snapshot.pendingSales.length > 0 ? (
        <div className="mt-4 rounded-2xl border border-rose-200 bg-rose-50 p-3">
          <p className="text-sm font-black text-rose-950">{t(lang, "dayCloseBlockedPendingTitle")}</p>
          <ul className="mt-2 space-y-2">
            {snapshot.pendingSales.map((row) => (
              <li key={row.saleId} className="rounded-xl bg-white/70 px-3 py-2 text-xs font-semibold text-rose-900">
                {row.label === "draft_cart"
                  ? t(lang, "dayCloseBlockedPendingDraft")
                  : tTemplate(lang, "dayCloseBlockedPendingRow", {
                      amount: row.totalUgx.toLocaleString(),
                      time: new Date(row.createdAt).toLocaleTimeString("en-UG", {
                        timeZone: "Africa/Kampala",
                        hour: "2-digit",
                        minute: "2-digit",
                      }),
          ...
          <Link to="/pending-sales" className="mt-2 inline-block text-xs font-black text-waka-700 underline">
            {t(lang, "dayCloseBlockedPendingFixLink")}
          </Link>
        </div>
      ) : null}
