...
function StatusIcon({ status }: { status: "pass" | "fail" | "warn" | "pending" }) {
  ...
  return <Circle className="h-5 w-5 shrink-0 text-stone-300" aria-hidden />;
}
...
export function CloseDayPreflightPanel({ lang, snapshot, loading }: Props) {
  ...
  return (
    <section className="rounded-3xl border border-stone-200 bg-white p-4 shadow-waka-sm">
      <h2 className="text-base font-black text-stone-950">{t(lang, "dayClosePreflightTitle")}</h2>
      <p className="mt-1 text-xs font-medium text-stone-500">{t(lang, "dayClosePreflightSub")}</p>
          ...
          {snapshot.items.map((item) => (
            <li
              ...
              className={`flex items-start gap-3 rounded-2xl px-3 py-2.5 ${
                item.status === "fail" ? "bg-rose-50" : item.status === "pass" ? "bg-emerald-50/80" : "bg-stone-50"
              }`}
            >
              <StatusIcon status={item.status} />
              <div className="min-w-0 flex-1">
                <p className="text-sm font-bold text-stone-900">{translate(item.labelKey)}</p>
                {item.detailKey && item.status !== "pass" ? (
                  <p className="mt-0.5 text-xs font-medium text-stone-600">{translate(item.detailKey)}</p>
                ) : null}
          ...
          <ul className="mt-2 space-y-2">
            {snapshot.hospitalitySessions.map((row) => (
              <li key={row.sessionId} className="rounded-xl bg-white/70 px-3 py-2">
                <p className="text-sm font-black text-stone-950">{row.label}</p>
                <p className="text-xs font-semibold text-stone-600">
                  {row.waiterLabel ? `${t(lang, "dayCloseWaiter")}: ${row.waiterLabel} · ` : ""}
                  {t(lang, "dayCloseSessionStatus")}: {row.status}
                </p>
                <p className="text-xs font-bold text-stone-800">UGX {row.amountUgx.toLocaleString()}</p>
              </li>
            ))}
          </ul>
        </div>
