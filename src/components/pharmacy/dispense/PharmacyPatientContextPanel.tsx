...
export function PharmacyPatientContextPanel({
  ...
  return (
    <div className="shrink-0 space-y-2 border-b border-border bg-teal-50/50 px-2 py-2 sm:px-3">
      ...
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div className="min-w-0">
          ...
          {patient.phone ? <p className="text-xs font-semibold text-muted-foreground">{patient.phone}</p> : null}
        </div>
        <Link
          to={`/pharmacy/patients/${patient.id}`}
          className="shrink-0 rounded-lg bg-card px-2 py-1 text-[10px] font-black text-teal-800 ring-1 ring-teal-200"
        >
      ...
      {chronic.length > 0 ? (
        <div>
          <p className="text-[10px] font-black uppercase text-muted-foreground">{t(lang, "pharmacyChronicTitle")}</p>
          <ul className="mt-1 flex flex-wrap gap-1">
            {chronic.map((m) => (
              <li key={m.id} className="rounded-lg bg-card px-2 py-0.5 text-[10px] font-bold text-foreground ring-1 ring-border">
                {m.productName}
              </li>
            ))}
          </ul>
      ...
      {timeline.length > 0 ? (
        <ul className="max-h-16 space-y-0.5 overflow-y-auto text-[10px] font-semibold text-muted-foreground">
          {timeline.map((ev) => (
