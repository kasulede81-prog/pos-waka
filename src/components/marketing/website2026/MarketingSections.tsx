          ...
          <Reveal key={item.title} delay={i * 0.05}>
            <HoverLift>
              <article className={clsx(mktCard, "h-full p-5")}>
                <p className="text-sm font-black text-waka-600 dark:text-waka-400">{item.title}</p>
        ...
        {MARKETING_BUSINESS_TYPES.map((biz, i) => (
          <Reveal key={`${biz.label}-${i}`} delay={i * 0.03}>
            <Link
              ...
              className={clsx(
                mktCard,
                "group flex min-h-[108px] flex-col justify-between p-4 transition hover:border-waka-500 hover:shadow-md dark:hover:border-waka-500/40",
              )}
            >
              <span className="text-2xl" aria-hidden>
                {biz.icon}
              </span>
              <span className="text-sm font-black text-mkt-text group-hover:text-waka-700 dark:group-hover:text-waka-700">
                {biz.label}
              </span>
            </Link>
          </Reveal>
...
export function MarketingShowcaseSection() {
  const panels = [
    ...
    { title: "Sell Screen", tone: "from-waka-600 to-waka-700", chips: ["Fast checkout", "Barcode", "Categories", "Offline"] },
          ...
          <Reveal key={pkg.id} delay={i * 0.06}>
            <HoverLift>
              <article className={clsx(mktCardLg, "flex h-full flex-col p-6")}>
                ...
                <p className="mt-3 text-3xl font-black text-waka-600 dark:text-waka-400">{pkg.priceLabel}</p>
                <ul className="mt-6 flex-1 space-y-2">
                  {pkg.includes.map((item) => (
                    <li key={item} className="flex gap-2 text-sm font-semibold text-mkt-text">
                      <span className="text-waka-600 dark:text-waka-400">✓</span>
                      {item}
                    </li>
                  ))}
                </ul>
                <Link
                  ...
                  className={clsx(
                    mktBtnSecondary,
                    "mt-6 min-h-[48px] border-waka-500 bg-waka-600 text-sm text-waka-600 hover:bg-waka-700 dark:border-waka-500/30 dark:bg-waka-500/30 dark:text-waka-400 dark:hover:bg-waka-700/50",
                  )}
                >
                  Contact sales
                </Link>
  ...
  return (
    <section id="pricing" className="py-16 sm:py-20">
        ...
        <div className="inline-flex rounded-full border border-mkt-border bg-mkt-bg-secondary p-1 text-sm font-black">
          <button
            ...
            className={clsx(
              ...
              !yearly ? "bg-mkt-card text-waka-600 shadow-sm dark:text-waka-400" : mktSubtext,
            )}
          >
            Monthly
          </button>
        ...
        {MARKETING_PLANS.map((plan, i) => (
          <Reveal key={plan.code} delay={i * 0.05}>
            ...
            >
              ...
              <p className="mt-2 text-3xl font-black text-waka-600 dark:text-waka-400">
                {plan.code === "free"
              ...
              <Link
                ...
                className={clsx(
                  ...
                  plan.popular ? clsx(mktBtnPrimary, "hover:bg-waka-700") : mktBtnSecondary,
                )}
              >
      ...
      <p className={clsx("mt-6 text-center text-sm font-medium", mktSubtext)}>
        <Link to="/pricing" className="font-black text-waka-600 underline-offset-2 hover:underline dark:text-waka-400">
...
export function MarketingFinalCtaSection() {
  return (
    <section className="overflow-hidden rounded-[2rem] bg-gradient-to-br from-waka-600 to-waka-700 px-6 py-12 text-white shadow-xl ring-1 ring-waka-500/30 sm:px-10 sm:py-14">
      <Reveal>
