...
export function MarketingHeroSection({ isAuthenticated }: Props) {
  return (
    <section className="relative overflow-hidden pt-4 pb-16 sm:pt-8 sm:pb-24 lg:pb-28">
      ...
      <div className="pointer-events-none absolute -right-24 top-1/4 -z-10 h-72 w-72 rounded-full bg-waka-600/10 blur-3xl transition-[background] duration-500 dark:bg-waka-500/20" />
      ...
      <div className="grid items-center gap-12 lg:grid-cols-[1.05fr_0.95fr] lg:gap-16">
        <Reveal>
          ...
          <h1 className={`mt-4 text-4xl font-black leading-[1.05] tracking-tight sm:text-5xl lg:text-[3.35rem] ${mktHeading}`}>
            Run Your Business <span className="text-waka-600 dark:text-waka-400">Smarter.</span>
          </h1>
          ...
          <ul className="mt-6 flex flex-wrap gap-3 text-sm font-bold text-mkt-text">
            <li className={mktChip}>
              <WifiOff className="h-4 w-4 text-waka-600 dark:text-waka-400" aria-hidden /> Offline first
            </li>
            <li className={mktChip}>
              <Cloud className="h-4 w-4 text-waka-600 dark:text-waka-400" aria-hidden /> Cloud sync
            </li>
            <li className={mktChip}>
              <HeadphonesIcon className="h-4 w-4 text-waka-600 dark:text-waka-400" aria-hidden /> Local support
            </li>
          </ul>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <Link
              ...
              className={clsx(mktBtnPrimary, "min-h-[52px] gap-2 px-7 py-3 text-base shadow-lg shadow-orange-600/25 hover:bg-waka-700")}
            >
              Start Free — UGX 0
            ...
            <a
              ...
              className="inline-flex min-h-[52px] items-center gap-2 rounded-2xl px-4 py-3 text-base font-bold text-mkt-text-secondary transition duration-300 hover:text-waka-700 dark:hover:text-waka-400"
            >
  ...
  return (
    <div className="relative mx-auto aspect-[4/3] max-h-[520px] w-full">
      ...
      <FloatCard delay={0.6} className={clsx(mktDeviceFrame, "absolute bottom-[6%] left-[18%] w-[28%]")}>
        ...
        <p className="text-lg font-black text-waka-600 dark:text-waka-400">+18%</p>
      ...
      <div className="absolute left-[8%] top-[2%] rounded-full border border-waka-500 bg-mkt-card px-3 py-1.5 text-[10px] font-black text-waka-600 shadow-md dark:border-waka-500/30 dark:text-waka-400">
        Offline ready
      </div>
    </div>
  );
