...
export function MarketingHeroSection({ isAuthenticated }: Props) {
  return (
    <section className="relative overflow-hidden pt-4 pb-16 sm:pt-8 sm:pb-24 lg:pb-28">
      ...
      <div className="pointer-events-none absolute -right-24 top-1/4 -z-10 h-72 w-72 rounded-full bg-waka-/10 blur-3xl transition-[background] duration-500 dark:bg-waka-/20" />
      ...
      <div className="grid items-center gap-12 lg:grid-cols-[1.05fr_0.95fr] lg:gap-16">
        <Reveal>
          ...
          <h1 className={`mt-4 text-4xl font-black leading-[1.05] tracking-tight sm:text-5xl lg:text-[3.35rem] ${mktHeading}`}>
            Run Your Business <span className="text-waka- dark:text-waka-">Smarter.</span>
          </h1>
          ...
          <ul className="mt-6 flex flex-wrap gap-3 text-sm font-bold text-mkt-text">
            <li className={mktChip}>
              <WifiOff className="h-4 w-4 text-waka- dark:text-waka-" aria-hidden /> Offline first
            </li>
            <li className={mktChip}>
              <Cloud className="h-4 w-4 text-waka- dark:text-waka-" aria-hidden /> Cloud sync
            </li>
            <li className={mktChip}>
              <HeadphonesIcon className="h-4 w-4 text-waka- dark:text-waka-" aria-hidden /> Local support
            </li>
          </ul>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <Link
              ...
              className={clsx(mktBtnPrimary, "min-h-[52px] gap-2 px-7 py-3 text-base shadow-lg shadow-orange-600/25 hover:bg-waka-")}
            >
              Start Free — UGX 0
            ...
            <a
              ...
              className="inline-flex min-h-[52px] items-center gap-2 rounded-2xl px-4 py-3 text-base font-bold text-mkt-text-secondary transition duration-300 hover:text-waka- dark:hover:text-waka-"
            >
...
function DeviceShowcase() {
  return (
    <div className="relative mx-auto aspect-[4/3] max-h-[520px] w-full">
          ...
          <div className="grid grid-cols-4 gap-1.5 p-2">
            {["Rice", "Sugar", "Soap", "Soda", "Bread", "Oil", "Eggs", "Milk"].map((p) => (
              <div key={p} className="rounded-lg bg-stone-800/80 p-2 text-center">
                <div className="mx-auto mb-1 h-6 w-6 rounded-md bg-waka-/30" />
      ...
      <FloatCard
        delay={1.2}
        className="absolute right-[2%] top-[18%] w-[34%] rounded-[1.4rem] border-[3px] border-stone-900 bg-stone-900 p-1.5 shadow-xl ring-1 ring-waka-/20 dark:ring-waka-/40"
      >
        <div className="overflow-hidden rounded-[1rem] bg-white">
          <div className="bg-waka- px-2 py-2 text-center text-[9px] font-black text-white">Today · UGX 842K</div>
      ...
      <FloatCard delay={0.6} className={clsx(mktDeviceFrame, "absolute bottom-[6%] left-[18%] w-[28%]")}>
        ...
        <p className="text-lg font-black text-waka- dark:text-waka-">+18%</p>
      ...
      <div className="absolute left-[8%] top-[2%] rounded-full border border-waka- bg-mkt-card px-3 py-1.5 text-[10px] font-black text-waka- shadow-md dark:border-waka-/30 dark:text-waka-">
        Offline ready
      </div>
    </div>
  );
