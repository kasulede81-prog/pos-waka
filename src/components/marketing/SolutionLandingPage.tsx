    ...
    <MarketingLayout lang={lang} setLang={setLang} isAuthenticated={isAuthenticated}>
      ...
      <article className="space-y-10">
        <header className="space-y-4">
          <p className="text-xs font-black uppercase tracking-[0.22em] text-waka-600">{content.eyebrow}</p>
          <h1 className="text-4xl font-black leading-tight text-mkt-text sm:text-5xl">{content.h1}</h1>
          <p className="max-w-3xl text-lg font-medium leading-relaxed text-mkt-text-secondary">{content.intro}</p>
          <div className="flex flex-wrap gap-3 pt-2">
            <Link
              to="/demo"
              className="inline-flex min-h-[48px] items-center rounded-2xl bg-waka-600 px-6 py-3 text-sm font-black text-white shadow-md hover:bg-waka-700"
            >
              {t(lang, "marketingCtaDemo")}
            </Link>
            <Link
              to="/register"
              className="inline-flex min-h-[48px] items-center rounded-2xl border-2 border-stone-200 bg-white px-6 py-3 text-sm font-black text-stone-900 hover:border-waka-500"
            >
        ...
        <section className="rounded-3xl border border-stone-100 bg-stone-950 p-6 text-white sm:p-8">
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            <Link
              to="/demo"
              className="inline-flex min-h-[48px] items-center rounded-2xl bg-waka-600 px-6 py-3 text-sm font-black text-white hover:bg-waka-700"
            >
              {t(lang, "marketingCtaDemo")}
            </Link>
            <Link
              to="/register"
              className="inline-flex min-h-[48px] items-center rounded-2xl border-2 border-stone-600 px-6 py-3 text-sm font-black text-white hover:border-waka-500"
            >
              {t(lang, "marketingCtaSignup")}
            </Link>
            <Link
              to="/contact"
              className="inline-flex min-h-[48px] items-center rounded-2xl px-6 py-3 text-sm font-black text-waka-600 underline-offset-4 hover:underline"
            >
              Talk to support
            </Link>
          </div>
        ...
        <section className="rounded-3xl border border-waka-500 bg-white p-6 shadow-waka-sm">
          ...
          <ul className="mt-4 flex flex-wrap gap-x-4 gap-y-2">
            {related.map((link) => (
              <li key={link.slug}>
                <Link to={link.path} className="text-sm font-black text-waka-600 underline-offset-4 hover:underline">
                  {link.label}
                </Link>
              </li>
            ))}
