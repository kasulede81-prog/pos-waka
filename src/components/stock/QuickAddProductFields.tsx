...
function StepLabel({ n, text }: { n: number; text: string }) {
  return (
    <span className="flex items-start gap-2.5">
      ...
      <span className="pt-0.5 text-sm font-bold leading-snug text-foreground">{text}</span>
    </span>
  );
}
...
export function QuickAddProductFields({
  ...
  const inputClass =
    variant === "page"
      ? "mt-2 min-h-[52px] w-full rounded-2xl border-2 border-border px-4 py-3 text-lg font-semibold outline-none ring-waka-200 focus:ring"
      : "mt-2 w-full rounded-2xl border-2 border-border px-4 py-4 text-xl font-semibold";
  const bigInputClass =
    variant === "page"
      ? "mt-2 min-h-[52px] w-full rounded-2xl border-2 border-border px-4 py-3 text-2xl font-black outline-none ring-waka-200 focus:ring"
      : "mt-2 w-full rounded-2xl border-2 border-border px-4 py-4 text-2xl font-black";
    ...
    <div className={variant === "page" ? "space-y-5" : "space-y-4"}>
      ...
      <label className="block">
        ...
        <select
          ...
          className={
            variant === "page"
              ? "mt-2 min-h-[48px] w-full rounded-2xl border-2 border-border bg-card px-4 py-3 text-base font-bold text-foreground outline-none ring-waka-200 focus:ring"
              : "mt-2 min-h-[48px] w-full rounded-2xl border-2 border-border bg-card px-4 py-3 text-base font-bold text-foreground"
          }
        >
          {unitOptions.map((u) => (
      ...
      {showOtherUnit ? (
        <label className="block pl-9">
          <span className="text-sm font-bold text-muted-foreground">{t(lang, "quickAddStep3Other")}</span>
          <input
      ...
      {!industryMode ? (
        <label className="block rounded-2xl border-2 border-waka-100 bg-waka-50/50 p-4">
          ...
          <p className="mt-1 pl-9 text-xs text-muted-foreground">{t(lang, "quickAddStep6Hint")}</p>
          <input
