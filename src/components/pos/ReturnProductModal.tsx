...
export function ReturnProductModal({ lang, open, sale, products, returnRecords = [], actorRole, onClose, onConfirm }: Props) {
  ...
  return (
    <PosScreenPortal>
      ...
      >
        <div
          className="flex max-h-[min(92vh,720px)] w-full max-w-md flex-col rounded-t-[1.75rem] bg-card shadow-2xl sm:rounded-3xl"
          onClick={(e) => e.stopPropagation()}
        >
          <div className="min-h-0 flex-1 overflow-y-auto p-5 pb-3">
            <h2 className="text-xl font-black text-foreground">{t(lang, "returnTitle")}</h2>
            <p className="mt-1 text-sm text-muted-foreground">{t(lang, "returnHint")}</p>
            ...
            <label className="mt-4 block text-sm font-bold text-foreground">
              ...
              <select
                ...
                className="mt-2 min-h-[48px] w-full rounded-2xl border-2 border-border px-3 text-base font-bold"
              >
                {pickList.map((item) => (
            ...
            <label className="mt-3 block text-sm font-bold text-foreground">
              ...
              <input
                ...
                inputMode="decimal"
                className="mt-2 min-h-[48px] w-full rounded-2xl border-2 border-border px-4 text-xl font-black"
              />
              {maxQty != null ? (
            ...
            {sale && refundBreakdown ? (
              <section className="mt-4 rounded-2xl border border-waka-200 bg-waka-50/50 p-3">
                <h3 className="text-sm font-black text-foreground">{t(lang, "returnRefundCustomerTitle")}</h3>
                <dl className="mt-2 space-y-1.5 text-sm">
                  <div className="flex justify-between gap-2">
                    <dt className="font-semibold text-muted-foreground">{t(lang, "refundBreakdownCustomerPaid")}</dt>
                    <dd className="font-black text-foreground">UGX {customerPaidUgx.toLocaleString()}</dd>
                  </div>
                  <div className="flex justify-between gap-2">
                    <dt className="font-semibold text-muted-foreground">{t(lang, "returnRecommendedRefund")}</dt>
                ...
                <label className="mt-3 block text-sm font-bold text-foreground">
                  ...
                  />
                  <span id="return-refund-hint" className="mt-1 block text-xs font-semibold text-muted-foreground">
                ...
                <button
                  ...
                  className="mt-3 flex w-full items-center justify-between rounded-xl border border-border bg-card px-3 py-2 text-xs font-black text-muted-foreground"
            ...
            <p className="mt-4 text-sm font-bold text-foreground">{t(lang, "returnReasonLabel")}</p>
