...
type Props = {
  ...
  onClose: () => void;
  onConfirm: (countedCashUgx: number) => { ok: boolean; errorKey?: string };
};

export function ShiftCloseModal({ lang, open, shift, onClose, onConfirm }: Props) {
  const [counted, setCounted] = useState("");

  const parts = useMemo(() => (shift ? shiftExpectedCashLabelParts(shift) : null), [shift]);
  ...
  return (
    <AppModalOverlay className="z-[64] flex items-end justify-center bg-black/55 sm:items-center" role="dialog" aria-modal onClick={onClose}>
      ...
      >
        <h2 className="text-xl font-black text-slate-900">{t(lang, "shiftCloseTitle")}</h2>
        ...
        <div className="mt-5 grid grid-cols-2 gap-2">
          ...
          <button
            type="button"
            disabled={countedN <= 0 && expected > 0}
            onClick={() => {
              const r = onConfirm(countedN);
              if (r.ok) onClose();
            }}
