      ...
      >
        ...
        <div className="mt-4 grid grid-cols-2 gap-2">
          <button
            ...
            className={`min-h-[44px] rounded-2xl border-2 text-sm font-black ${
              mode === "fixed" ? "border-waka-500 bg-waka-100 text-waka-950" : "border-slate-200 text-slate-700"
            }`}
          >
          ...
          <button
            ...
            className={`min-h-[44px] rounded-2xl border-2 text-sm font-black ${
              mode === "percent" ? "border-waka-500 bg-waka-100 text-waka-950" : "border-slate-200 text-slate-700"
            }`}
          >
        ...
        {mode === "fixed" ? (
          <label className="mt-4 block">
            <span className="text-sm font-bold text-slate-700">{t(lang, "cartDiscountAmountLabel")}</span>
            <MoneyInput
              value={fixedInput}
              onChange={(e) => setFixedInput(e.target.value.replace(/\D/g, "").slice(0, 10))}
              autoFocus
              className="mt-2 min-h-[56px] w-full rounded-2xl border-2 border-slate-200 px-4 text-center text-2xl font-black"
            />
          </label>
        ) : (
          <label className="mt-4 block">
            <span className="text-sm font-bold text-slate-700">{t(lang, "cartDiscountPercentLabel")}</span>
            <input
              type="text"
              inputMode="numeric"
              value={percentInput}
