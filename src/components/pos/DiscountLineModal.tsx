...
export function DiscountLineModal({ lang, open, line, onClose, onApply }: Props) {
  ...
  return (
    <PosScreenPortal>
        ...
        footer={
          <div className="grid grid-cols-2 gap-2">
            <button
              type="button"
              onClick={onClose}
              className="min-h-[52px] rounded-2xl border-2 border-border py-3 text-base font-bold text-muted-foreground"
            >
              {t(lang, "cancel")}
            </button>
            <button
      ...
      >
        <p className="line-clamp-3 text-2xl font-black text-foreground">{line.name}</p>
        <p className="mt-1 text-base font-semibold text-muted-foreground">
          {t(lang, "discountOriginal")}:{" "}
          <span className="font-black text-foreground">UGX {list.toLocaleString()}</span>
        </p>
