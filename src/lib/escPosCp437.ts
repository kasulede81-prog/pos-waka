/**
 * Deterministic ESC/POS text encoding for PC437 (ESC t 0).
 *
 * The target MP-58N-8012 self-test reports CP437. UTF-8 TextEncoder is not
 * a compatible substitute: multi-byte sequences print as garbage.
 *
 * ASCII 0x20–0x7E is unchanged. A small typography fold maps common Unicode
 * punctuation onto ASCII equivalents. Unsupported code points become '?'
 * rather than a misleading glyph (e.g. € is not rewritten as £).
 */

const REPLACEMENT = 0x3f; // '?'

/** IBM CP437 glyphs for bytes 0x80–0xFF (Unicode code points). */
const CP437_HIGH: readonly number[] = [
  0x00c7, 0x00fc, 0x00e9, 0x00e2, 0x00e4, 0x00e0, 0x00e5, 0x00e7, 0x00ea, 0x00eb, 0x00e8, 0x00ef, 0x00ee, 0x00ec,
  0x00c4, 0x00c5, 0x00c9, 0x00e6, 0x00c6, 0x00f4, 0x00f6, 0x00f2, 0x00fb, 0x00f9, 0x00ff, 0x00d6, 0x00dc, 0x00a2,
  0x00a3, 0x00a5, 0x20a7, 0x0192, 0x00e1, 0x00ed, 0x00f3, 0x00fa, 0x00f1, 0x00d1, 0x00aa, 0x00ba, 0x00bf, 0x2310,
  0x00ac, 0x00bd, 0x00bc, 0x00a1, 0x00ab, 0x00bb, 0x2591, 0x2592, 0x2593, 0x2502, 0x2524, 0x2561, 0x2562, 0x2556,
  0x2555, 0x2563, 0x2551, 0x2557, 0x255d, 0x255c, 0x255b, 0x2510, 0x2514, 0x2534, 0x252c, 0x251c, 0x2500, 0x253c,
  0x255e, 0x255f, 0x255a, 0x2554, 0x2569, 0x2566, 0x2560, 0x2550, 0x256c, 0x2567, 0x2568, 0x2564, 0x2565, 0x2559,
  0x2558, 0x2552, 0x2553, 0x256b, 0x256a, 0x2518, 0x250c, 0x2588, 0x2584, 0x258c, 0x2590, 0x2580, 0x03b1, 0x00df,
  0x0393, 0x03c0, 0x03a3, 0x03c3, 0x00b5, 0x03c4, 0x03a6, 0x0398, 0x03a9, 0x03b4, 0x221e, 0x03c6, 0x03b5, 0x2229,
  0x2261, 0x00b1, 0x2265, 0x2264, 0x2320, 0x2321, 0x00f7, 0x2248, 0x00b0, 0x2219, 0x00b7, 0x221a, 0x207f, 0x00b2,
  0x25a0, 0x00a0,
];

const UNICODE_TO_CP437 = new Map<number, number>();
for (let i = 0; i < 0x80; i++) UNICODE_TO_CP437.set(i, i);
for (let i = 0; i < CP437_HIGH.length; i++) UNICODE_TO_CP437.set(CP437_HIGH[i]!, 0x80 + i);

const TYPOGRAPHY_FOLDS: Readonly<Record<string, string>> = {
  "\u00a0": " ",
  "\u202f": " ",
  "\u2018": "'",
  "\u2019": "'",
  "\u201a": "'",
  "\u201b": "'",
  "\u2032": "'",
  "\u201c": '"',
  "\u201d": '"',
  "\u201e": '"',
  "\u201f": '"',
  "\u2033": '"',
  "\u2013": "-",
  "\u2014": "-",
  "\u2015": "-",
  "\u2212": "-",
  "\u2026": "...",
};

export type EscPosCp437EncodeResult = {
  bytes: number[];
  /** Distinct original characters that have no CP437 mapping after typography fold. */
  unsupported: string[];
};

export function isEscPosCp437Mapped(codePoint: number): boolean {
  return UNICODE_TO_CP437.has(codePoint);
}

function foldTypography(text: string): string {
  let out = "";
  for (const ch of text) {
    out += TYPOGRAPHY_FOLDS[ch] ?? ch;
  }
  return out;
}

export function encodeEscPosCp437(text: string): EscPosCp437EncodeResult {
  const folded = foldTypography(text);
  const bytes: number[] = [];
  const unsupported: string[] = [];
  const seen = new Set<string>();
  for (const ch of folded) {
    const cp = ch.codePointAt(0) ?? 0;
    const mapped = UNICODE_TO_CP437.get(cp);
    if (mapped != null) {
      bytes.push(mapped);
      continue;
    }
    bytes.push(REPLACEMENT);
    if (!seen.has(ch)) {
      seen.add(ch);
      unsupported.push(ch);
    }
  }
  return { bytes, unsupported };
}

export function encodeEscPosCp437Bytes(text: string): number[] {
  return encodeEscPosCp437(text).bytes;
}
