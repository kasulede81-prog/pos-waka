    ...
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-2">
        <div className="rounded-2xl border border-stone-200 bg-white p-3 shadow-sm">
          <p className="text-[10px] font-bold uppercase text-stone-500">{t(lang, "ipPaymentsPeriod")}</p>
        ...
        <div className="rounded-2xl border border-rose-100 bg-rose-50/50 p-3 shadow-sm">
          <p className="text-[10px] font-bold uppercase text-stone-500">{t(lang, "ipStatOutstanding")}</p>
      ...
      <div className="flex flex-wrap gap-2">
        <select value={supplierFilter} onChange={(e) => setSupplierFilter(e.target.value)} className="rounded-xl border border-stone-200 bg-white px-3 py-2 text-xs font-bold">
      ...
      {owingSuppliers.length > 0 ? (
        <section>
          <h3 className="mb-2 text-xs font-black uppercase tracking-wide text-stone-500">{t(lang, "ipUpcomingPayments")}</h3>
      ...
      <section>
        <h3 className="mb-2 text-xs font-black uppercase tracking-wide text-stone-500">{t(lang, "supplierPaymentHistory")}</h3>
        {payments.length === 0 ? (
          <p className="text-sm font-semibold text-stone-500">{t(lang, "supplierPaymentEmpty")}</p>
        ) : (
          <ul className="space-y-2">
            {payments.map((pay) => (
              <li key={pay.id} className="rounded-2xl border border-stone-100 bg-white px-4 py-3 shadow-sm">
                <div className="flex items-center justify-between gap-2">
                  <div>
                    <p className="font-black text-stone-900">{supplierNameById.get(pay.supplierId) ?? "—"}</p>
