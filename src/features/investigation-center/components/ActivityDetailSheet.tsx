...
function DetailRow({ label, value }: { label: string; value: string | null | undefined }) {
  ...
  return (
    <div className="flex items-start gap-3 border-b border-border py-3 last:border-b-0">
      <p className="w-28 shrink-0 text-[11px] font-black uppercase tracking-wide text-muted-foreground">{label}</p>
      <p className="min-w-0 flex-1 text-sm font-semibold text-foreground break-words">{value}</p>
    </div>
  );
}
      ...
      footer={
        <div className="space-y-2">
          <div className="grid grid-cols-2 gap-2">
            <button type="button" onClick={onCopy} className="inline-flex min-h-[44px] items-center justify-center gap-2 rounded-2xl border-2 border-border text-sm font-black text-foreground">
              <Copy className="h-4 w-4" aria-hidden />
              {t(lang, "icCopyDetails")}
            </button>
            <button type="button" onClick={onShare} className="inline-flex min-h-[44px] items-center justify-center gap-2 rounded-2xl border-2 border-border text-sm font-black text-foreground">
              <Share2 className="h-4 w-4" aria-hidden />
              {t(lang, "icShare")}
            </button>
          </div>
          <div className="grid grid-cols-2 gap-2">
            <button type="button" onClick={onPrint} className="inline-flex min-h-[44px] items-center justify-center gap-2 rounded-2xl border-2 border-border text-sm font-black text-foreground">
      ...
      <div className="space-y-4">
        <div className="rounded-2xl border border-border bg-muted/70 p-4">
          <div className="flex items-start justify-between gap-3">
            <div>
              <h3 className="text-lg font-black text-foreground">{auditActionLabel(lang, entry.action)}</h3>
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            {before ? (
              <div className="rounded-2xl border border-border bg-card p-3">
                <p className="text-[10px] font-black uppercase tracking-wide text-muted-foreground">{t(lang, "auditExportColBefore")}</p>
                <p className="mt-1 text-sm font-black text-foreground">{before}</p>
              </div>
            ) : null}
            {after ? (
        ...
        <div className="rounded-2xl border border-border bg-muted/80">
          <button
            type="button"
