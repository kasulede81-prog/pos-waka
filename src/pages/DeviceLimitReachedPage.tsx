...
export function DeviceLimitReachedPage({ lang, onSignOut }: Props) {
  ...
  return (
    <div className="mx-auto min-h-screen max-w-lg space-y-6 bg-gradient-to-b from-orange-50 to-stone-50 px-4 py-8">
      <div className="text-center">
        ...
        <h1 className="mt-4 text-2xl font-black text-stone-950">{t(lang, "deviceLimitTitle")}</h1>
        <p className="mt-2 text-sm font-medium text-stone-600">{t(lang, "deviceLimitSub")}</p>
      </div>

      <section className="rounded-2xl border border-stone-200 bg-white p-4 shadow-sm">
        <p className="text-xs font-bold uppercase tracking-wide text-stone-500">{t(lang, "deviceLimitPackageLabel")}</p>
        <p className="mt-1 text-lg font-black text-stone-950">{planDisplayName(ctx, block?.result.plan_name)}</p>
        <p className="mt-2 text-sm font-bold text-stone-800">{usageLine}</p>
      </section>

      {!isOwner ? (
        <section className="rounded-2xl border border-amber-200 bg-amber-50 p-4">
          <p className="text-sm font-bold text-amber-950">{t(lang, "deviceLimitNonOwnerTitle")}</p>
          <p className="mt-1 text-sm font-medium text-amber-900">{t(lang, "deviceLimitNonOwnerBody")}</p>
        </section>
      ) : null}

      <section>
        <h2 className="text-sm font-black text-stone-800">{t(lang, "deviceLimitDevicesHeading")}</h2>
        {loading ? (
          <p className="mt-2 text-sm text-stone-500">{t(lang, "connectedDevicesLoading")}</p>
        ) : activeDevices.length === 0 ? (
          <p className="mt-2 text-sm text-stone-500">{t(lang, "connectedDevicesEmpty")}</p>
        ) : (
          <ul className="mt-3 space-y-3">
            {activeDevices.map((device) => {
              ...
              return (
                <li key={device.id} className="rounded-2xl border border-stone-200 bg-white p-4 shadow-sm">
                  <p className="text-base font-black text-stone-950">{name}</p>
                  <p className="text-sm font-medium text-stone-600">{formatDevicePlatformLabel(device.platform)}</p>
                  ...
                  {isOwner && !isCurrent ? (
                    <button
                      ...
                      className="mt-3 min-h-[44px] w-full rounded-xl border border-red-200 bg-red-50 px-4 text-sm font-bold text-red-800 disabled:opacity-50"
                    >
                      {busyId === device.id
      ...
      <div className="flex flex-col gap-3">
        {isOwner ? (
          <button
            ...
            className="min-h-[48px] rounded-xl bg-stone-900 px-4 text-sm font-bold text-white"
          >
        ...
        <button
          ...
          className="min-h-[48px] rounded-xl border border-stone-300 bg-white px-4 text-sm font-bold text-stone-800"
        >
