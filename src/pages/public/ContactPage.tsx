    ...
    <MarketingLayout lang={lang} setLang={setLang} isAuthenticated={isAuthenticated}>
      ...
      <article className="space-y-8">
        <header className="space-y-3">
          <p className="text-xs font-black uppercase tracking-[0.22em] text-waka-700">Contact</p>
          <h1 className="text-4xl font-black text-foreground">Talk to the Waka team</h1>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <ContactCard icon={Mail} title="Email">
            {WAKA_SUPPORT_EMAILS.map((email) => (
              <a key={email} href={`mailto:${email}`} className="block text-waka-800 underline-offset-2 hover:underline">
                {email}
              </a>
            ))}
          </ContactCard>
          <ContactCard icon={Phone} title="Phone">
            <p className="text-muted-foreground">+256 792 521 711</p>
            <p className="mt-1 text-xs text-muted-foreground">WhatsApp preferred for faster support</p>
          </ContactCard>
          <ContactCard icon={MapPin} title="Office address">
            <p>{WAKA_OFFICE_STREET}</p>
            <p className="mt-1">{WAKA_OFFICE_CITY}, Uganda</p>
            <p className="mt-2 text-xs text-muted-foreground">Postal: {WAKA_COMPANY_POSTAL_ADDRESS}</p>
          </ContactCard>
        ...
        <section className="overflow-hidden rounded-3xl border border-border bg-muted">
          <div className="flex aspect-[16/9] flex-col items-center justify-center gap-2 p-8 text-center">
            <MapPin className="h-10 w-10 text-waka-600" aria-hidden />
            <p className="text-sm font-black text-foreground">Map — Kampala office</p>
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="mt-2 text-sm font-bold text-waka-800 underline"
            >
              Open in Google Maps →
            </a>
          </div>
...
function ContactCard({
  ...
  return (
    <div className="rounded-3xl border border-waka-100 bg-card p-5 shadow-waka-sm">
      <div className="flex gap-3">
        <Icon className="mt-0.5 h-5 w-5 shrink-0 text-waka-600" aria-hidden />
        <div>
