    ...
    <div className="space-y-4 pb-10">
      ...
      <article className="rounded-2xl border border-teal-200 bg-teal-50/60 p-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-base font-black text-foreground">{t(lang, "pilotSupportCenterSub")}</p>
            <p className="mt-1 text-sm text-muted-foreground">{t(lang, "pilotSupportCenterHelp")}</p>
          </div>
        </div>
      </article>

      <label className="block rounded-2xl border border-border bg-card p-4">
        <span className="text-sm font-black text-foreground">{t(lang, "pilotSupportIssueLabel")}</span>
        <textarea
          value={issueNote}
      ...
      {Capacitor.isNativePlatform() ? (
        <button
          ...
          className="flex w-full cursor-pointer items-center gap-3 rounded-2xl border border-dashed border-border bg-muted px-4 py-3 text-left"
        >
        ...
        <label className="flex cursor-pointer items-center gap-3 rounded-2xl border border-dashed border-border bg-muted px-4 py-3">
      ...
      <div className="grid gap-2 sm:grid-cols-2">
        ...
        <button
          ...
          disabled={!exportPayload}
          className="inline-flex h-11 items-center justify-center gap-2 rounded-xl border-2 border-teal-700 bg-card text-sm font-black text-teal-900 disabled:opacity-50"
        >
      ...
      {events.length > 0 ? (
        <section className="rounded-2xl border border-border bg-card p-4">
          <p className="text-sm font-black text-foreground">{t(lang, "pilotEventLogTitle")}</p>
          <ul className="mt-2 max-h-48 space-y-1 overflow-y-auto text-xs text-muted-foreground">
            {events.map((e) => (
              <li key={e.id} className="truncate">
                <span className="font-bold text-foreground">{e.kind}</span> · {e.summary}
              </li>
            ))}
          </ul>
        </section>
