      ...
      <main className="flex flex-1 flex-col px-8 py-6">
        {state === "thanks" ? (
          <div className="flex flex-1 flex-col items-center justify-center text-center">
            ...
            <p className="mt-4 text-2xl font-semibold text-muted-foreground">{t(lang, "customerDisplayThanksSub")}</p>
          </div>
        ) : (
          <>
            <ul className="flex-1 space-y-3 overflow-y-auto">
              {(payload?.lines ?? []).map((line, idx) => (
                <li key={`${line.name}-${idx}`} className="flex items-baseline justify-between gap-4 border-b border-stone-800 pb-3">
                  ...
                  <span className="text-xl font-semibold text-muted-foreground">UGX {line.lineTotalUgx.toLocaleString()}</span>
                </li>
              ))}
              {!payload?.lines?.length ? (
                <li className="text-2xl font-semibold text-muted-foreground">{t(lang, "customerDisplayEmpty")}</li>
              ) : null}
            </ul>
            <footer className="mt-6 border-t border-stone-700 pt-6">
              <div className="flex items-end justify-between">
                <span className="text-2xl font-bold text-muted-foreground">{t(lang, "customerDisplayTotal")}</span>
