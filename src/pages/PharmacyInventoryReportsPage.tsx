  ...
  return (
    <EnterprisePageContainer>
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-3xl font-black tracking-tight text-foreground">{t(lang, "pharmacyInventoryReports")}</h1>
          <p className="mt-1 text-base font-medium text-muted-foreground">{t(lang, "pharmacyInventoryReportsSub")}</p>
        </div>
        <Link
          to="/pharmacy/expiry"
      ...
      <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {tiles.map((tile) => (
          <div key={tile.labelKey} className="rounded-2xl border border-border bg-card p-4 shadow-waka-sm">
            <p className="text-xs font-black uppercase text-muted-foreground">{t(lang, tile.labelKey)}</p>
            <p className="mt-1 text-2xl font-black text-foreground">{tile.value}</p>
          </div>
        ))}
      </section>

      <section className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-3xl border border-border bg-card p-4 shadow-waka-sm">
          <h2 className="text-lg font-black text-foreground">{t(lang, "pharmacyDashTopMedicines")}</h2>
        ...
        <div className="rounded-3xl border border-border bg-card p-4 shadow-waka-sm">
          <h2 className="text-lg font-black text-foreground">{t(lang, "pharmacyReportSlowMovers")}</h2>
          <ul className="mt-3 space-y-2">
