  ...
  if (!sessionId || !session) {
    return (
      <div className="page-content-pad">
        ...
        <p className="text-sm font-semibold text-slate-500">{t(lang, "invalid")}</p>
      </div>
    );
  }
    ...
    <div className="page-content-pad space-y-5 pb-24">
      ...
      {canCount ? (
        <input
          ...
          className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-base font-semibold"
        />
      ) : null}
          ...
          return (
            <li key={line.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              <div className="flex items-start justify-between gap-2">
                <p className="text-base font-black text-slate-950">{line.productName ?? line.productId}</p>
              ...
              <dl className="mt-2 grid grid-cols-2 gap-2 text-xs font-semibold text-slate-600">
                <div>
                  <dt>{t(lang, "inventoryCountSnapshotStock")}</dt>
                  <dd className="text-sm font-black text-slate-900">{line.expectedQtySnapshot}</dd>
                </div>
                {showReview && currentStock != null ? (
                  <div>
                    <dt>{t(lang, "inventoryCountCurrentStock")}</dt>
                    <dd className={`text-sm font-black ${drift ? "text-amber-700" : "text-slate-900"}`}>
                      {currentStock}
                    </dd>
                  </div>
                ) : null}
                {line.countedQty != null ? (
                  <>
                    <div>
                      ...
                      <dd className="text-sm font-black text-slate-900">{line.countedQty}</dd>
                    </div>
                    <div>
                      <dt>{t(lang, "inventoryCountVariance")}</dt>
                      <dd
                        className={`text-sm font-black ${line.varianceQty < 0 ? "text-rose-700" : line.varianceQty > 0 ? "text-emerald-700" : "text-slate-900"}`}
                      >
                        {line.varianceQty >= 0 ? "+" : ""}
                        {line.varianceQty}
                      </dd>
              ...
              {canCount ? (
                <div className="mt-3 flex flex-col gap-2 sm:flex-row">
                  <input
                    ...
                    className="min-w-0 flex-1 rounded-xl border border-slate-200 px-3 py-3 text-lg font-black tabular-nums"
                  />
                  <input
                    type="text"
