...
Deno.serve(async (req) => {
  ...
  const { data: prep, error: prepErr } = await userClient.rpc("owner_permanently_delete_own_account", {
  ...
  if (authDelErr) {
    return json({
      ...
      partial: true,
      message:
        "Your shop data was removed but the login could not be deleted. Contact Waka support or retry — you may not be able to register again until the login is removed.",
  ...
  return json({
    ...
    sales_deleted: j.sales_deleted,
    message: "Your account, shop data, and login were permanently deleted. You can register again with the same email.",
  });
});
