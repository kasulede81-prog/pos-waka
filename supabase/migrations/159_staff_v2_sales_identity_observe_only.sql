-- STAFF-V2-PHASE-3: observe-only sold_by_user_id dual-write.
-- Does not change created_by, RLS, PIN, invitations, or shop_members.
-- Malformed / unknown Auth UUIDs become NULL and must not abort the sale.

create or replace function public.staff_v2_observe_sold_by_user_id (p_sale jsonb)
returns uuid
language plpgsql
stable
security definer
...
revoke all on function public.staff_v2_observe_sold_by_user_id (jsonb) from public;
revoke all on function public.staff_v2_observe_sold_by_user_id (jsonb) from anon, authenticated;
...
begin
  ...
  v_sold_by := public.staff_v2_observe_sold_by_user_id (v_sale);
