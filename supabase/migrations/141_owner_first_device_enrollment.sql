-- Phase 20.6: Owner-first authentication — shop owners auto-enroll devices in one transaction.
-- Staff devices still require owner approval (pending state unchanged for non-owners).
  ...
  if v_status = 'disconnected'::public.shop_device_status and v_approval = 'approved' then
    ...
    if not v_shop_has_devices then
      ...
      perform public.audit_shop_device_event(
        ...
        'Owner device auto-enrolled on login',
        v_fp,
        jsonb_build_object('owner_enrolled', true, 'platform', p_platform)
      );
    else
      v_active := public.count_shop_active_devices(p_shop_id, null);
      select dl.device_limit, dl.plan_code, dl.plan_name
...
-- Ensure activation aligns: owners never stay pending.
