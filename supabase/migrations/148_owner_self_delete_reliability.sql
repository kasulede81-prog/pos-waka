-- Phase 39.3: DB verification without auth.users, org-name confirmation,
-- explicit RESTRICT child deletes, non-destructive readiness probe.

create or replace function public.owner_deletion_confirmation_matches (
  p_typed text,
...
as $$
  select
    upper(trim(coalesce(p_typed, ''))) = 'DELETE PERMANENTLY'
    or (
      nullif(trim(coalesce(p_organization_name, '')), '') is not null
      and upper(trim(p_typed)) = upper(trim(p_organization_name))
    );
$$;

revoke all on function public.owner_deletion_confirmation_matches (text, text) from public;
grant execute on function public.owner_deletion_confirmation_matches (text, text) to authenticated;
...
begin
  ...
  -- Hospitality: table_sessions.sale_id and table_id are ON DELETE RESTRICT.
  -- Kitchen tickets cascade from table_sessions.
  if cardinality(v_shop_ids) > 0 then
    delete from public.table_sessions ts
    where ts.shop_id = any (v_shop_ids);
    get diagnostics v_table_sessions_removed = row_count;
  end if;
  ...
  -- Transfers CASCADE from organization_id but RESTRICT shop deletes; delete first.
...
begin
  ...
  if not public.owner_deletion_confirmation_matches (p_confirmation, v_org_name) then
    return jsonb_build_object (
      'ok',
      false,
      'error',
      ...
      'detail',
      'Type DELETE PERMANENTLY or the exact organization name to confirm.'
    );
  end if;
  ...
  ) || jsonb_build_object ('shop_id', v_shop_id, 'shop_name', v_shop_name, 'organization_name', v_org_name);
end;
$$;
...
begin
  ...
  select exists (
    ...
    where n.nspname = 'public'
      and p.proname = 'owner_permanently_delete_own_account'
  ...
  select exists (
    ...
    join pg_namespace n on n.oid = p.pronamespace
    where n.nspname = 'public'
      and p.proname = 'certified_hard_delete_organization_execute'
  )
  into v_execute_ok;

  return jsonb_build_object (
    'ok',
    v_rpc_ok and v_execute_ok,
    'rpc',
    'owner_permanently_delete_own_account',
    'execute_rpc',
    v_execute_ok,
    'confirmation_contract',
    'delete_permanently_or_organization_name',
    'destructive',
    false,
    'migration',
    '148_owner_self_delete_reliability'
  );
end;
$$;
