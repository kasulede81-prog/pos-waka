-- PURCHASE-VOID-STOCK-1.0: durable exactly-once stock reversal for purchase voids.
-- Depends on: 083 (inventory_movement_uuid), 168/172 (_apply_durable_stock_delta pattern).
-- Additive only — does NOT mutate historical stock or backfill guessed void movements.
-- Does NOT change WAC, prices, payables, debt, cash, or purchase RECEIVE (166).
--
-- Durable grain: (shop_id, 'purchase_void', purchase_id, product_id)
-- A purchase may be voided at most once; purchase_id uniquely identifies the void event.

-- One cloud purchase-void stock reversal per purchase × product.
create unique index if not exists inventory_movements_purchase_void_product_unique
  on public.inventory_movements (shop_id, reference_type, reference_id, product_id)
  where reference_type = 'purchase_void' and reference_id is not null;

-- Extend the internal durable primitive to accept purchase_void (still not granted to clients).
...
begin
  ...
  -- Preserve existing purchase-void floor semantics (generic push used greatest(..., 0)).
...
-- Public purchase-void stock RPC. Shop authority is the product row, not client trust alone.
-- Identity: (shop_id, 'purchase_void', purchase_id, product_id).
-- Stock leg only — payables remain on the existing purchase/supplier sync path.
create or replace function public.shop_apply_purchase_void_stock (
  p_shop_id uuid,
  p_payload jsonb
)
returns jsonb
...
begin
  ...
  v_purchase_id := nullif(
    coalesce(p_payload ->> 'purchase_id', p_payload ->> 'reference_id'),
    ''
  )::uuid;
  v_delta := coalesce((p_payload ->> 'delta')::numeric, 0);
  v_note := nullif(p_payload ->> 'note', '');

  if v_product_id is null or v_purchase_id is null then
    return jsonb_build_object('ok', false, 'error', 'invalid_payload');
  end if;

  -- Purchase void reverses inbound stock (negative). Reject zero/positive.
  if v_delta >= 0 then
    return jsonb_build_object('ok', false, 'error', 'invalid_delta');
  end if;
...
revoke all on function public.shop_apply_purchase_void_stock (uuid, jsonb) from public;
grant execute on function public.shop_apply_purchase_void_stock (uuid, jsonb) to authenticated;
