-- Final hardening: make business-profile save fully self-contained.
-- This avoids calling bootstrap_owner_workspace (which can vary across deployed DB versions)
-- and removes ambiguous-column edge cases around organization_id.
...
begin
  ...
  -- Ensure profile exists and satisfies strict schemas (full_name can be NOT NULL on some deployments).
  ...
  if not exists (
    ...
    where sp.code = 'starter' and sp.is_active
    limit 1;
